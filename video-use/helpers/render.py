...
def extract_segment_multicam(
    ...
    if layout == "graphic":
        ...
        fc = f"[0:v]{tonemap}{_scale_cover(W, H)},fps=24{grade}[vout];[0:a]{af}[aout]"
        ...
        if portrait:
            fc = (
                ...
                f"[pcv]fps=24{grade}[vout];[1:a]{af}[aout]"
            )
        else:
            fc = f"[0:v]{tonemap}{screen_pre}{_scale_pad(W, H)},fps=24{grade}[vout];[1:a]{af}[aout]"
        io = [
...
def build_final_composite(
    ...
    over the base audio (with a limiter to avoid clipping). If there are no
    overlays, no subtitles and no sfx, just copy base to out.
    """
    sfx = sfx or []
    ...
    # ---- audio: mix timed SFX over the base mic track ----------------------
    if has_sfx:
        sfx_labels = []
        for j, s in enumerate(sfx):
            inp = 1 + len(overlays) + j
            at_ms = max(0, int(round(float(s.get("at", 0)) * 1000)))
            gain = float(s.get("gain_db", 0))
            filter_parts.append(f"[{inp}:a]adelay={at_ms}:all=1,volume={gain}dB[sfx{j}]")
            sfx_labels.append(f"[sfx{j}]")
        # duration=first: lock output length to the base (voice) track. amix's
        # default `longest` mis-extends the timeline when SFX are adelay-padded to
    ...
    print(f"  overlays: {len(overlays)}, subtitles: {'yes' if has_subs else 'no'}, sfx: {len(sfx)}")
    subprocess.run(cmd, check=True, stdout=subprocess.DEVNULL, stderr=subprocess.PIPE)
...
def main() -> None:
    ...
    if args.no_loudnorm:
        ...
        build_final_composite(base_path, overlays, subs_path, out_path, edit_dir, sfx=sfx, sub_margin=sub_margin)
    else:
        # Composite to a temp file, then run loudnorm → final output
        tmp_composite = out_path.with_suffix(".prenorm.mp4")
        build_final_composite(base_path, overlays, subs_path, tmp_composite, edit_dir, sfx=sfx, sub_margin=sub_margin)
